...
export default function CampaignsPage() {
  ...
  return (
    <div className="space-y-8 p-6 sm:p-8 animate-in fade-in duration-500 max-w-[1600px] mx-auto">
      ...
      {(quickCampaignApisUnavailable && !sendReadiness && !sendReadinessQuery.loading) ? (
        <div className="rounded-xl border border-amber-300/50 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:bg-amber-900/20 dark:text-amber-300">
          بعض وظائف الحملات غير متاحة في نسخة Convex الحالية. تم تعطيل الإجراءات الحساسة حتى نشر الدوال المطلوبة.
        </div>
      ) : null}
                    ...
                    <Card key={campaign._id} className="p-0 overflow-hidden border-none ring-1 ring-border/50 shadow-none hover:bg-muted/30 transition-colors">
                      <div className="p-5 flex flex-col sm:flex-row sm:items-center gap-6">
                        ...
                        <div className="flex items-start gap-4 flex-1">
                          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${campaign.status === 'COMPLETED' ? 'bg-success/10 text-success' :
                    ...
                    <div className="space-y-1.5">
                      {dayCampaigns.map(c => (
                        <Link key={c._id} href={`/campaigns/${c._id}`}>
                          <div className={`text-xs p-2 rounded-lg border truncate transition-all hover:scale-[1.02] active:scale-95 cursor-pointer ${c.status === 'COMPLETED' ? 'bg-success/5 border-success/20 text-success-foreground' :
