...
export const sendMessage = action({
  ...
  handler: async (ctx, args) => {
    ...
    console.log(`[WhatsApp] Sending payload to ${args.to}:`, JSON.stringify(payload));
  ...
  handler: async (ctx, args) => {
    ...
    if (value.messages) {
      ...
      for (const message of value.messages) {
        ...
        const messageId = await ctx.runMutation((internal as any).messages.saveMessage, {
    ...
    if (value.statuses) {
      ...
      for (const status of value.statuses) {
        ...
        const success = await ctx.runMutation((internal as any).messages.updateMessageStatus, {
